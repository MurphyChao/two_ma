...
void CheckForOpen()
{
    ...
    {
        printf("position volume: %d", PositionGetDouble(POSITION_VOLUME));
        if (BuyOpenCondition())
        {
    ...
    if (signal != WRONG_VALUE)
    {
            ...
            Bars(_Symbol, _Period) > 100)
        {
            ...
            double tp = oreo.GetTP(signal, open);
            double sl = oreo.GetSL(signal);
...
void CheckForClose()
{
    ...
    bool signal = false;
    if (PositionGetDouble(POSITION_VOLUME))
    {
